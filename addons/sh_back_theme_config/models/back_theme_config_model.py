...
class sh_back_theme_config_settings(models.Model):
    ...
    def search_primary_var_template(self):
        if self.env.ref('sh_backmate_theme.sh_assets_primary_variables'):
            return self.env.ref('sh_backmate_theme.sh_assets_primary_variables').active
    
    def deactivate_primary_variable_scss(self):
        if self.env.ref('sh_backmate_theme.sh_assets_primary_variables'):
            self.env.ref('sh_backmate_theme.sh_assets_primary_variables').write({'active':False})
        if self.env.ref('sh_backmate_theme.sh_assets_background_color_scss'):
            self.env.ref('sh_backmate_theme.sh_assets_background_color_scss').write({'active':True})
        return True
    
    def activate_primary_variable_scss(self):
        if self.env.ref('sh_backmate_theme.sh_assets_primary_variables'):
            self.env.ref('sh_backmate_theme.sh_assets_primary_variables').write({'active':True})
        if self.env.ref('sh_backmate_theme.sh_assets_background_color_scss'):
            self.env.ref('sh_backmate_theme.sh_assets_background_color_scss').write({'active':False})
        return True
