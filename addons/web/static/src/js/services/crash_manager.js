...
var CrashManager = AbstractService.extend({
    ...
    show_error: function (error) {
        ...
        var dialog = new dialogClass(this, {
            title: _.str.capitalize(error.type) || _t("Hive Error"),
        }, error);
...
var RedirectWarningHandler = Widget.extend(ExceptionHandler, {
    ...
    display: function() {
        ...
        new WarningDialog(this, {
            title: _.str.capitalize(error.type) || _t("Hive Warning"),
            buttons: [
...
core.crash_registry.add('hive.exceptions.RedirectWarning', RedirectWarningHandler);

function session_expired(cm) {
    return {
        display: function () {
            cm.show_warning({type: _t("Hive Session Expired"), message: _t("Your Hive session expired. Please refresh the current web page.")});
        }
    };
}
core.crash_registry.add('hive.http.SessionExpiredException', session_expired);
